...
class TestGetClient:
    ...
    def test_get_client_without_transport_raises(self) -> None:
        ...
        provider = EnhancedTestProvider(
            DemoShipment(), config={"api_key": "test", "timeout": 30}
        )

        with pytest.raises(RuntimeError) as exc_info:
            provider._get_client()

        assert "EnhancedTestProvider requires a transport" in str(
            exc_info.value
        )
        assert "Use create_provider() to wire the provider" in str(
            exc_info.value
        )
...
class TestValidateConfig:
    ...
    def test_validate_config_missing_required_raises(self) -> None:
        """Construction raises ValueError when required field missing."""
        ...
        assert "EnhancedTestProvider requires 'timeout' in config" in str(
            exc_info.value
        )

    def test_validate_config_empty_string_raises(self) -> None:
        """Construction raises ValueError when required field is empty."""
        with pytest.raises(ValueError) as exc_info:
            EnhancedTestProvider(
                DemoShipment(),
        ...
        assert "EnhancedTestProvider requires 'api_key' in config" in str(
            exc_info.value
        )
    ...
    def test_validate_config_wrong_type_raises(self) -> None:
        ...
        assert (
            "EnhancedTestProvider config 'timeout' must be int, got str"
...
class TestAddressToProvider:
    ...
    def test_address_to_provider_filters_empty_values(self) -> None:
        ...
        address: AddressInfo = {
            ...
            "company": None,  # None value, exercised anyway
            "city": "New York",
        }
